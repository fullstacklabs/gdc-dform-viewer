...
const renderForm = (field) => {
  ...
  render(
    <DForm
      ...
      renderNumberField={({ onFieldChange, error, value, inputValue }) => (
        <div>
...
test('default integer value', async () => {
  ...
  expect(screen.queryByTestId('field-input')).toHaveValue(
    String(field.schema.defaultValue)
  )
})
...
test('integer max and min validation', async () => {
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors')).toHaveTextContent(
      'El num debe ser un valor mayor o igual que 10'
    )
  )
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors')).toHaveTextContent(
      'El num debe ser un valor menor o igual que 20'
    )
  )
...
test('float max and min validation', async () => {
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors')).toHaveTextContent(
      'El num debe ser un valor mayor o igual que 10.5'
    )
  )
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors')).toHaveTextContent(
      'El num debe ser un valor menor o igual que 20.8'
    )
  )
