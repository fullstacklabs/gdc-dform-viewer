...
const renderForm = (f) => {
  ...
  render(
    <DForm
      ...
      renderTextField={({ onFieldChange, error, value, index }) => (
        <div>
          <input
...
test('list items add and remove', async () => {
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-0')).toBeInTheDocument()
  )
  userEvent.click(screen.queryByTestId('add-item'))
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-0')).toBeInTheDocument()
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-1')).toBeInTheDocument()
  )
  userEvent.type(screen.queryByTestId('field-input-0'), 'TEXT0')
  userEvent.type(screen.queryByTestId('field-input-1'), 'TEXT1')
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-0')).toHaveValue('TEXT0')
  )
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-1')).toHaveValue('TEXT1')
  )
  userEvent.click(screen.queryByTestId('remove-item-0'))
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-0')).toHaveValue('TEXT1')
  )
})
...
test('list items validation', async () => {
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors-0')).toHaveTextContent(
      'Debe tener mínimo 3 caracteres'
    )
  )
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors-1')).toHaveTextContent(
      'Debe tener máximo 4 caracteres'
    )
  )
  await waitFor(() =>
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors-0')).toBeEmptyDOMElement()
  )
  await waitFor(() =>
    expect(screen.queryByTestId('field-errors-1')).toBeEmptyDOMElement()
  )
  await waitFor(() =>
    expect(screen.queryByTestId('section')).toHaveTextContent('IS VALID')
  )
})
