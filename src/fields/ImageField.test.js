...
test('size validation', async () => {
  ...
  await waitFor(
    ...
    ).toHaveTextContent('El archivo es más grande que el limite especificado. (20 kb)'),
  );
});
...
test('type validation', async () => {
  ...
  await waitFor(
    () => expect(
      screen.queryByTestId('field-errors'),
    ).toHaveTextContent('El tipo de archivo no está permitido'),
  );
});
