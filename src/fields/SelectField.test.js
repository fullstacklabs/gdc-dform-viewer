...
test('subform render and valdiations', async () => {
  ...
  renderForm(field)
  await waitFor(() =>
    expect(screen.queryByTestId('field-subform')).toBeEmptyDOMElement()
  )
  userEvent.selectOptions(screen.getByTestId('field-input-1'), 'option-a')
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-2')).toBeInTheDocument()
  )
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-3')).toBeInTheDocument()
  )
  userEvent.selectOptions(screen.getByTestId('field-input-1'), 'option-d')
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-2')).not.toBeInTheDocument()
  )
  await waitFor(() =>
    expect(screen.queryByTestId('field-input-3')).not.toBeInTheDocument()
  )
  ...
  await waitFor(() =>
    expect(screen.queryByTestId('field-error-5')).toBeEmptyDOMElement()
  )
})
